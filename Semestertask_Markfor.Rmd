...
The  RSS is the best for the full model, that was to expect since a model with more parameters always leads to smaller scores. The AIC is also better for the bigger model. The behaviour of the AIC is decreasing and after a minima it increases again therefore the AIC probably isn't at a minima. 
...
This is wonderfully consistent with modeling, we can see that the small model still leads to good predictions and we don't have to go to higher dimentions if a relation can be explained very well with two variables. 
