...
```{r lm_two_output,out.width='80%',fig.dim=c(5,3), fig.cap="Left: Visualization 3d, Rigth: Results from the model comparison", echo=FALSE}
...
In the end we can compare the 3 models with the earlier mentioned measurements. The Distinction is made for measurements concerning the model itself how well it fits the data and the predictions which were made with the model for the test data. 

The  RSS is the best for the full model, that was to expect since a model with more parameters always leads to smaller scores. The AIC is also better for the bigger model. The behaviour of the AIC is decreasing and after a minimum it increases again therefore the AIC probably isn't at a minimum.
...
*[2] Linear Regression and its assumptions, Januar 24,2020,Manish Sharma, https://towardsdatascience.com/linear-regression-and-its-assumptions-ef6e8db4904d*
