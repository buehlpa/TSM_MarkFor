...
https://www.kaggle.com/aungpyaeap/fish-market*Note: the output is not shown to reduce space in the document full code and generation of the document can be found on https://github.com/buehlpa/TSM_MarkFor*
...
```
From the summary output we get the following model $Weight_{i}=(-6.000 + 4.9368 * Width_{i})^{2}$. The explained Variance measurement $R^{2}$ is with $0.9683$ very high. the $AIC$ for this model is $ 171.6$. The p-values for the both parameters are also significant. By inspecting the residuals in Figure \ref{fig:resid_perch} we can see that the model assumptions are met. The residuals seem to follow a normal distribution, have constant mean and there are no points which are crucial to shift the model, maybe the variance is slightly inflated.
